...
hmmer_vis.domain_architectures_view = function() {
	...
	// The cbak returned
	var domain_architectures_view = function(div, data,query_architecture_id, match_da) {
		...
		var svg_div = li.append("div").attr('class','domGraphics').attr('id', function(d,i){
			dom_data.push(d);
			dom_arch_ids.push(i);
			return match_da ? "dom_ma" : "dom_"+i;
		})
		for (var id of dom_arch_ids) {
				   console.log(id);
				   var domain_data = dom_data[id];
				   var target_id = match_da? "#dom_ma": "#dom_"+id;
				// add pfam graphics here
					var chart = new PfamGraphic(target_id, domain_data);
					// var chart = new PfamGraphic();
					chart.render();
					var new_width = $(target_id).parent().width();
					new_width = 450;
